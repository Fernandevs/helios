...
const PROJECTS: ProjectEntity[] = [
  ...
  },
  {
    ...
    longDescription:
      'Graviton was built to replace an aging NGINX-based gateway that could not support the routing complexity of a microservices mesh grown to 40 services. The new gateway is written in Go and runs as a sidecar alongside each service, forming a control-plane/data-plane split: a central Go binary handles routing configuration and policy distribution, while per-service Envoy proxies handle the actual packet forwarding.\n\nRate limiting is implemented at three granularities — global, per-route, and per-consumer — using a token bucket algorithm backed by Redis with Lua scripting for atomic operations. Adaptive circuit breakers use a lightweight gradient boosting model (compiled to WebAssembly for portability) that recalibrates open/closed thresholds every 60 seconds based on rolling error rate, latency distribution, and downstream queue depth — reducing false positives by 61% compared to static thresholds.\n\nAll service-to-service calls are instrumented with OpenTelemetry spans, propagated via W3C Trace Context headers, and exported to a Tempo backend for distributed tracing. A real-time admin dashboard built in React displays per-route p50/p95/p99 latencies, circuit breaker states, and rate limit consumption with a 2-second refresh interval, enabling on-call engineers to diagnose issues without SSH access.',
    highlights: [
      '2 M req/sec sustained across 40 microservices',
      '61% fewer circuit breaker false positives with ML adaptive thresholds',
      'Three-level rate limiting: global, per-route, per-consumer',
      'Full distributed tracing via OpenTelemetry + W3C Trace Context',
      'Real-time admin dashboard with 2-second refresh on all metrics',
    ],
  },
  {
    id: '6',
    ...
    longDescription:
      'Stellar Auth eliminates the password entirely — not just with OAuth delegation but at the cryptographic level. Authentication proofs are generated using Groth16 ZK-SNARKs: the client proves knowledge of a credential without revealing it, and the server verifies the proof in under 4 ms using a precomputed verification key. This makes credential interception or database leakage cryptographically useless.\n\nSession tokens are hardware-bound via WebAuthn Level 3 attestation. During registration, the authenticator (YubiKey, Touch ID, or Android Strongbox) generates a resident key pair; the private key never leaves the secure enclave. Subsequent authentications produce a device assertion that the server verifies against the stored public key. Sessions are managed in Redis with sliding expiration, device fingerprint binding, and geographic anomaly scoring — logins from unusual locations trigger a step-up challenge automatically.\n\nThe anomaly detection layer runs a streaming Isolation Forest model on login events, scoring each attempt in real time against a feature vector that includes IP reputation, device entropy, time-of-day distribution, and behavioral biometrics from the authentication gesture. False positive rate in beta testing was 0.3% with a true positive rate of 97.4% on synthetic attack scenarios. The service is currently in closed beta with three design partners.',
    highlights: [
      'ZK-SNARK proof verification in under 4 ms (Groth16)',
      'Private keys never leave hardware secure enclaves (WebAuthn L3)',
  ...
  },
  {
    ...
    longDescription:
      'Pulsar Event Stream was designed to unify five disparate Kafka clusters that had grown independently across business units, each with different schema conventions, retention policies, and access control models. The migration to Apache Pulsar enabled a multi-tenancy model where each business unit gets isolated namespaces with independent quota enforcement while sharing the underlying broker infrastructure — reducing total cluster cost by 38%.\n\nReal-time processing pipelines are written in Scala with Apache Flink, using the Flink-Pulsar connector for source and sink. Exactly-once semantics are guaranteed end-to-end through Flink checkpoints coordinated with Pulsar transaction API, tolerating broker restarts and consumer crashes without message duplication or loss. Backpressure propagation is handled automatically: when a downstream ClickHouse sink slows below a threshold, Flink reduces source parallelism and publishes a metric that triggers horizontal pod autoscaling on the Kubernetes deployment.\n\nA central schema registry enforces Avro schema compatibility (backward, forward, and full) across all 240 active topics. Schema evolution is governed by a CI gate: any topic schema change that would break existing consumers blocks the deployment pipeline and notifies the owning team via Slack. This eliminated a class of silent data corruption bugs that previously required multi-day forensic investigations to detect.',
    highlights: [
      ...
      'Unified 5 legacy Kafka clusters with zero data loss migration',
    ],
  },
];
